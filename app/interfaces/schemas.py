...
from app.domain.models.upgrade import AppliedUpgrade, UpgradeSpec
...
class AppliedUpgradeRead(BaseModel):
    name: str
    applied_at: str
    price: float | None = None

    @classmethod
    def from_entity(cls, upgrade: AppliedUpgrade) -> "AppliedUpgradeRead":
        return cls(
            name=upgrade.name,
...
class ServerRead(BaseModel):
    ...
    applied_upgrades: list[AppliedUpgradeRead] = Field(default_factory=list)

    @classmethod
    def from_entity(cls, server: Server, vm_password: str | None = None) -> "ServerRead":
        return cls(
            id=server.id,
            ...
            disk_storage=server.disk_storage,
            primary_ip=server.primary_ip,
            vm_password=vm_password or server.vm_password,
            expire_in_days=server.expire_in_days,
            expire_at=server.expire_at.isoformat() if server.expire_at else None,
            created_at=server.created_at.isoformat(),
            status=server.status,
            external_id=server.external_id,
            applied_upgrades=[AppliedUpgradeRead.from_entity(upgrade) for upgrade in server.applied_upgrades],
        )
...
class ServerPasswordResetRequest(BaseModel):
    pass
