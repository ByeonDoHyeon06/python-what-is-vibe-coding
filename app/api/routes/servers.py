...
def get_server(
    ...
    server = refresher.refresh_by_id(server_id)
    if not server:
...
def start_server(
    server_id: UUID,
    payload: ServerOwnerRequest,
    control: ControlServerPower = Depends(get_server_power_control),
):
    try:
        server = control.start(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
...
def stop_server(
    ...
    try:
        server = control.stop(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def reboot_server(
    ...
    try:
        server = control.reboot(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def reset_server(
    ...
    try:
        server = control.reset(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def shutdown_server(
    ...
    try:
        server = control.shutdown(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def suspend_server(
    ...
    try:
        server = control.suspend(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def resume_server(
    ...
    try:
        server = control.resume(server_id, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
...
def extend_server(
    ...
    try:
        server = extender.extend(
            server_id, additional_days=payload.additional_days, user_id=payload.user_id
        )
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
...
def upgrade_server(
    ...
    try:
        server = upgrader.apply(server_id, upgrade_name=payload.upgrade, user_id=payload.user_id)
        return ServerRead.from_entity(server)
    except ValueError as exc:
        status = 404 if "not found" in str(exc).lower() else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc
